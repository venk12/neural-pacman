...
def lsl_to_datetime(lsl_timestamp):
    """
    Convert LSL timestamp to a human-readable datetime format.

    Args:
        lsl_timestamp (float): The LSL timestamp to convert.

    Returns:
        datetime: A datetime object representing the timestamp in UTC.
    """
    # Convert the LSL timestamp (which is in seconds since epoch) to a datetime object
    dt = datetime.fromtimestamp(lsl_timestamp, tz=timezone.utc)
    return dt
